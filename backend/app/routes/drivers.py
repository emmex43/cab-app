...
def get_driver_profile():
    ...
    try:
        ...
        if not driver: return jsonify({'error': 'Driver profile not found'}), 404
...
def accept_ride(ride_id):
    try:
        ...
        if ride.status != 'pending': return jsonify({'error': 'Ride is not available'}), 400
        ...
        socketio.emit('ride_accepted', {
            'ride_id': ride.id,
            'student_id': ride.user_id,
            'driver_name': driver.user.fullname,
            'vehicle': f"{driver.vehicle_description} ({driver.vehicle_type})"
        })
